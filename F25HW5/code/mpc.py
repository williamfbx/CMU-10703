...
class MPC:
    ...
    def predict_next_state_model(self, states, actions):
        ...
        # every row of states is same
        # actions are different?

        if not torch.is_tensor(states):
            states = torch.tensor(states, dtype=torch.float32, device=self.model.device)
        if not torch.is_tensor(actions):
            actions = torch.tensor(actions, dtype=torch.float32, device=self.model.device)
        
        next_states = torch.zeros_like(states)
        
        # expand actions to match the number of particles [popsize * num_particles, action_dim]
        actions_expanded = actions.repeat_interleave(self.num_particles, dim=0)

        inputs = torch.cat([states, actions_expanded], dim=1) # [popsize * num_particles, state_dim + action_dim]
        ensemble_outputs = self.model(inputs) # [num_nets, (mean tensor, logvar tensor)] where each tensor is [popsize * num_particles, state_dim]

        for i in range(states.shape[0]): # for every particle and every action seq candidate
            # TS1: randomly select one model from the ensemble for each particle
            selected_net_idx = torch.randint(0, self.num_nets, (1,)).item()
            means, logvars = ensemble_outputs[selected_net_idx]
            mean = means[i]
            logvar = logvars[i]
        
            std = torch.exp(0.5 * logvar)
            delta = mean + std * torch.randn_like(mean)
            next_states[i] = states[i] + delta.squeeze()
